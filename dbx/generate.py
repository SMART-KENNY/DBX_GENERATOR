...
def generate_sql(
    ...
    if p_header == "true":
        ...
        for _, row in df.iterrows():
            ...
            if field_name == "dbx_process_dttm":
                line = f"now() {field_name}"
            elif data_type.lower() in {"timestamp", "date"} or field_name in {"file_name", "file_id"}:
                line = f"{field_name}"
            else:
...
def main() -> None:
    ...
    parent_output_path = r"C:\Users\KLMembrano\OneDrive - PLDT\DATA-BRICKS\dbx\output\\"
    parent_template_path = r"C:\Users\KLMembrano\OneDrive - PLDT\DATA-BRICKS\dbx\templates\\"
    parent_trash_path = r"C:\Users\KLMembrano\OneDrive - PLDT\DATA-BRICKS\dbx\trash\\"
